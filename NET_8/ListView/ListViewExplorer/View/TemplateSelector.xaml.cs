...
namespace ListViewExplorer
{
    ...
    public class ItemTemplateSelector : DataTemplateSelector
    {
        ...
        {
            var frameElement = container as FrameworkElement;
            ...
            try
            {
                if (lvItem.Content is TeamA)
                    template = frameElement.FindResource("Template1") as DataTemplate;
                else if (lvItem.Content is TeamB)
                    template = frameElement.FindResource("Template2") as DataTemplate;
            }
            catch
            {
            }
