...
{
  public delegate bool ProcessVectorItem(VectorItemBase vector);
  ...
  public static class Extension
  {
      ...
      ProcessVectorItem processVector)
    {
      using (StreamReader sr = new StreamReader(stream))
      {
        ...
        List<VectorItemBase> vects = KmlReader.Read(s);

        vl.BeginUpdate();
        foreach (VectorItemBase vect in vects)
        {
    ...
    public static void LoadKMZ(this VectorLayer vl, Stream stream, bool centerAndZoom,
      ProcessVectorItem processVector)
    {
      using (StreamReader sr = new StreamReader(stream))
      {
        List<VectorItemBase> vects = KmzReader.Read(stream);
    ...
    public static Rect GetBounds(this IEnumerable<VectorItemBase> items)
    {
      Rect bounds = Rect.Empty;
      foreach (VectorItemBase item in items)
