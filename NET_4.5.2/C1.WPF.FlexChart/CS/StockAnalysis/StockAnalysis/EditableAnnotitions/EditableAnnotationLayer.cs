...
namespace StockAnalysis.EditableAnnotitions
{
    ...
    public class EditableAnnotationLayer : AnnotationLayer
    {
        ...
        private Point _start = new Point(-1, -1), _end;          //For Adding Annotations
        ...
        private void OnMouseDown(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            ...
            if (pos.Y > ClientAreaHeight)
            {
                pos.Y = ClientAreaHeight;
            }
        ...
        private void OnMouseMove(object sender, System.Windows.Input.MouseEventArgs e)
        {
            ...
            Dispatcher.BeginInvoke(new Action(() =>
            {
                if (AllowMove && SelectedAnnotation != null && _isDragging && _flexChart.PlotRect.Contains(e.GetPosition(FlexChart)))
                {
                    ...
                    if (_newPoint.Y > ClientAreaHeight)
                    {
                        _newPoint.Y = ClientAreaHeight;
                    }
                    var diff = new Point
                    {
                    ...
                    else if (SelectedAnnotation is Polygon)
                    {
                        ...
                        for (int i = 0; i < polygon.Points.Count; i++)
                        {
                            ...
                            if (pt.Y > ClientAreaHeight)
                            {
                                pt.Y = ClientAreaHeight;
                            }
                    ...
                    else
                    {
                        ...
                        if (location.Y > ClientAreaHeight)
                        {
                            location.Y = ClientAreaHeight;
                        }
        ...
        private DependencyObject FindParentWindow(DependencyObject obj, Type type)
        {
            var parent = VisualTreeHelper.GetParent(obj);
            if (parent == null || type.IsAssignableFrom(parent.GetType()))
            {
                return parent;
            }
            else
            {
                return FindParentWindow(parent, type);
            }
        }
        ...
        private void GetAnnotationEditorData(AnnotationBase anno)
        {
            ...
            if (anno is Text)
            {
                ...
                {
                    contentStyle.FontFamily = contentStyle.FontFamily == null ? style.FontFamily : contentStyle.FontFamily;
            ...
            if (contentStyle != null)
            {
                ...
                editStyle.FontFamily = contentStyle.FontFamily;
        ...
        private void AddAnnotation(Point p)
        {
            ...
            try
            {
                ...
                if (_annotationToAdd is C1.WPF.Chart.Annotation.Shape)
                {
                    _annotationToAdd.SetValue(C1.WPF.Chart.Annotation.Shape.ContentProperty, NewAnnotationType.Name);
                }
        ...
        private void ApplyStyle(AnnotationBase anno, bool isText = false)
        {
            ...
            if (isText)
            {
                anno.Style.FontFamily = ViewModel.ViewModel.Instance.EditingAnnotationStyle.FontFamily;
            ...
            else
            {
                ...
                {
                    FontFamily = ViewModel.ViewModel.Instance.AnnotationStyle.FontFamily,
    ...
    public abstract class TextEditor : UserControl
    {
        ...
        public void AcceptChanges(string content)
        {
            ...
            if (Annotation is Shape)
            {
                Annotation.SetValue(Shape.ContentProperty, content);
            }
            else if (Annotation is Line)
            {
                Annotation.SetValue(Line.ContentProperty, content);
            }
            else if (Annotation is Text)
            {
                Annotation.SetValue(Text.ContentProperty, content);
            }
        }
        ...
        private void SetAnnotationEditorData(AnnotationBase anno, Data.AnnotationStyle style)
        {
            ...
            if (anno is Text)
            {
                ...
                anno.Style.FontFamily = style.FontFamily;
            ...
            else if (anno is Shape)
            {
                ...
                shape.ContentStyle.FontFamily = style.FontFamily;
