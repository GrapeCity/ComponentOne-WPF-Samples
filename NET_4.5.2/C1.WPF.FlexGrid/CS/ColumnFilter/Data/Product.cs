...
namespace ColumnFilter
{
        ...
        IEditableObject
    {
        ...
        public bool? Special
        {
            get { return (bool?)GetValue("Special"); }
            set { SetValue("Special", value); }
        }

        [Display(Name = "Launched")]
        public DateTime? Launched
        {
            get { return (DateTime?)GetValue("Launched"); }
            set { SetValue("Launched", value); }
        }
        ...
        public static IList<Product> GetProducts(int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                p.Price = rnd.NextDouble() * 1000;
                p.Weight = rnd.NextDouble() * 100;
                p.Cost = rnd.NextDouble() * 600;
                p.Volume = 500 + rnd.NextDouble() * 4000;
                p.Discontinued = rnd.NextDouble() < .1;
                p.Rating = rnd.Next(0, 5);
                if (rnd.NextDouble() > .2)
                {
                    p.Special = rnd.NextDouble() > 0.5;
                }
                if (rnd.NextDouble() > .2)
                {
                    p.Launched = DateTime.Today.AddDays(rnd.Next(-3000, -300));
                }
                list.Add(p);
            }
            return list;
